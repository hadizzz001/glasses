...
const Cart = () => {
    ...
    const getItemPrice = (item) => {
        if (item.type === 'collection' && selectedOptions[item._id]) {
            const colorObj = item.color.find(c => c.id === selectedOptions[item._id].colorId);
            const sizeObj = colorObj?.sizes.find(s => s.size === selectedOptions[item._id].size);
            return (sizeObj?.price || 0) * (localQuantities[item._id] || 1);
        } else {
            return (item.discount || item.price || 0) * (localQuantities[item._id] || 1);
        }
    };
                                ...
                                cart.map((obj) => (
                                    <div key={obj._id} className="Checkout_Cart_LineItems_LineItem">
                                        ...
                                        <div className="Checkout_Cart_LineItems_LineItem_Details myGray">
                                            ...
                                            {obj.color && obj.color.length > 0 && selectedOptions[obj._id] && (
                                                <div className="flex gap-2 mt-1">
                                                    <span className="myGray">Color:</span>
                                                    <span className="myGray font-bold">
                                                        {obj.color.find(c => c.id === selectedOptions[obj._id].colorId)?.title}  
                                                    </span>

                                                    <span className="myGray ml-4">Size:</span>
                                                    <span className="myGray font-bold">
                                                        {selectedOptions[obj._id].size}
                                                    </span>
                                                </div>
                                            )}
